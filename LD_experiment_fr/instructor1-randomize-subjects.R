...
# Importez les données de l'ensemble des participants qui seront les sujets de l'expérience.
...
# Définissez la "graine de randomisation" (seed) pour que vous obteniez le même résultat chaque fois que vous exécutez ce code.
set.seed(42)

# randomize using the randomizr package function block_ra, set the probability of treatment to be 50% within each block
# Randomisez à l'aide de la fonction block_ra du package randomizr et définissez la probabilité de traitement à 50 % pour chaque bloc.
participants <- 
  participants |> 
  mutate(treatment = block_ra(blocks = Gender, prob = 0.5))

# confirm balance across blocking covariate
# Confirmez l'équilibre par rapport aux covariables de bloc.
...
# Enregistrez les données pour mettre en œuvre l'expérience
