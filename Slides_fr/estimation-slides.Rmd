...
## Points clés sur l'estimation I
  ...
  - Pour en savoir plus sur $\tau_{i}$, nous pouvons traiter $\tau_{i}$ comme un **estimande** ou une quantité cible à estimer (discutée ici) ou comme quantité cible hypothétique (voir module sur les tests d'hypothèses).
...
## Points clés sur l'estimation II
 ...
 - La clé de l'estimation pour l'inférence causale consiste à choisir un estimande qui vous aide à en savoir plus sur votre question théorique ou politique. Ainsi, on pourrait utiliser l'ATE mais aussi d'autres estimandes courants tels que l'effet d'intention de traiter (intent to treat effect, ITT), l'effet moyen local du traitement (local average treatment effect, LATE), l'effet causal moyen pour ceux qui se conforment au traitement (complier average causal effect, CACE), l'effet moyen du traitement sur les traités (average treatment effect on the treated, ATT) ou l'ATE pour certains sous-groupes.
...
## Points clés sur l'estimation III
 ...
 - Nous utilisons l'**erreur type** pour produire des **intervalles de confiance** et des **p-valeurs**
   afin de commencer par un estimateur et terminer par un test d'hypothèse.

  - Différentes randomisations produiront différentes valeurs du même estimateur ciblant le même estimande. L'**erreur type** résume la variabilité de cet estimateur.
...
En réalité, nous n'observons qu'un seul des résultats potentiels.
...
Calculez maintenant votre estimation avec les mêmes estimateurs en utilisant une randomisation **différente**. Notez que les réponses diffèrent. Les estimateurs estiment le *même estimande* mais ils travaillent maintenant avec une randomisation différente.
...
## Cela a le design de base, l'estimande, puis nos deux estimateurs (diff_means et diff_means_topcoded)
...
Résumé :
  ...
  - Un bon estimateur performe bien quelle que soit la randomisation particulière d'un design donné.
    Et *bien performer* peut signifier "être sans biais" et/ou "avoir une faible erreur quadratique moyenne"
...
Quel est l'estimande de l'**effet moyen du traitement** dans une expérience aléatoire par bloc ?
...
Et il serait naturel d'*estimer* cette quantité en y introduisant ce qu'on peut calculer :
...
## déclarer un nouveau design de base qui inclut b, l'indicateur de bloc
base_design_blocks <-
...
## Dans les expériences aléatoires par grappe, les unités sont randomisées par grappe pour le traitement {.allowframebreaks}
...
- Lorsque la taille de la grappe est liée aux résultats potentiels, l'estimateur habituel de différences des moyennes est biaisé. [Lien](https://declaredesign.org/blog/bias-cluster-randomized-trials.html)


## Estimation de l'erreur type pour l'ATE dans des expériences aléatoires par grappe {.allowframebreaks}

- **Inférences statistiques trompeuses :** L'erreur type par défaut sous-estimera généralement la précision dans de tels designs et produira ainsi des tests avec des taux de faux positifs trop élevés (ou, de manière équivalente, des taux de couverture pour les intervalles de confiance trop faibles).
...
thepop <- declare_population(clus_id = add_level(
            ...
            # Chaque grappe a un niveau moyen différent (u) et une variabilité de base différente (sd de u)
...
```{r showresclus1}
kableExtra::kable(res_clus[,c(1,5:7)], digits=2,booktabs=TRUE,linesep="",caption="Performances de l'estimateur et des tests pour 5000 simulations du design randomisé par grappe pour différents estimateurs et intervalles de confiance")
```
...
```{r showresclus2}
kableExtra::kable(res_clus[,c(1:3)], digits=3,booktabs=TRUE,linesep="",caption="Performances de l'estimateur et des tests pour 5000 simulations du design randomisé par grappe pour différents estimateurs et intervalles de confiance")
```
...
# mais en tenant compte de l'erreur type, etc.
...
# assignation et valeurs de résultat liées à Z
...
```{r showresbin1}
kableExtra::kable(res_bin, digits=3,booktabs=TRUE,linesep="",caption="Performances de l'estimateur et des tests pour 5000 simulations des différents estimateurs et intervalles de confiance pour un résultat binaire et un design totalement randomisé.")
...
- Les estimandes causaux contrefactuels sont des fonctions non observées des résultats potentiels.
...
- $Z_i$ est l'assignation aléatoire à une visite ($Z_i=1$) ou non ($Z_i=0$).
...
(x_1 \ldots x_p) \arrow[from=2-1,to=1-1, "\text{0 (comme si randomisé)}"]  \arrow[from=2-1,to=1-2] \arrow[from=2-1,to=1-4]
\end{tikzcd}
\end{center}
...
Mais, dans ce design, $\bar{y}_{Z=1}=\bar{y}_{1}$ est divisé en plusieurs parties : le résultat de ceux qui ont répondu à la porte (conformistes et toujours-preneurs et non-conformistes).
...
(x_1 \ldots x_p) \arrow[from=2-1,to=1-1, "\text{-.006 (comme si randomisé)}"]  \arrow[from=2-1,to=1-2, ".06"] \arrow[from=2-1,to=1-4, ".48"]
\end{tikzcd}
\end{center}
