...
Notez que nous devons *connaître* les résultats potentiels et l'assignation de traitement afin de savoir si notre estimateur fait un bon travail.
...
Si notre théorie suggère que les effets devraient différer selon le groupe, comment pouvons-nous évaluer les preuves pour ou contre une telle affirmation?
 ...
 - We can **plan** for such an assessment by (1) **pre-registering specific subgroup analyses**
   (whether or not we block on that group in the design phase)
   and (2) making sure to measure group membership during baseline data collection pre-treatment

   Nous pouvons **planifier** une telle évaluation en (1) **pré-enregistrant des analyses de sous-groupes spécifiques**
   (que nous bloquions ou non ce groupe dans la phase de design)
...
## Effets différents selon les groupes II
 - If we have not planned ahead, subgroup-specific analyses can be useful as
   ...
   positive rates.
   Si on n'a pas prévu à l'avance, les analyses par sous-groupes peuvent être utiles en mode exploratoire mais pas pour avoir des confirmatoires:
...
Nous **ne devrions pas utiliser des groupes formés par traitement**. (Il s'agit soit d'une "analyse de médiation" soit d'un "conditionnement sur des variables post-traitement" et mérite son propre module).
...
## declare_reveal is basically the same as declare_potential_outcomes. Je pense qu'ils ont cela ici pour faire face aux situations de données manquantes ou de non-conformité.
# thereveal <- declare_reveal(Y, Z)
