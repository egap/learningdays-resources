...
- Historiquement, la causalité inverse et le biais de variable omise ont posé problémae pour la recherche en sciences sociales qui prétend expliquer les causes.
...
- Les expériences nous aident à en apprendre davantage sur les affirmations de causalité contrefactuelles ou dues sur la manipulation.
...
1.  "X cause Y" n'implique pas nécessairement que W et V ne causent pas Y. X fait partie de l'histoire, pas ce n'est pas toute l'histoire. (Toute l'histoire n'est pas nécessaire pour savoir si X cause Y).
...
- Nous avons l'effet de traitement pour chaque individu.

- Notez l'hétérogénéité des effets du traitement au niveau individuel.

- Mais nous n'avons au plus qu'un résultat potentiel pour chaque individu, ce qui signifie que nous ne connaissons pas ces effets de traitement.
...
$\overline{\tau_i} = \frac{1}{N} \sum_{i=1}^N ( Y_i(1)-Y_i(0) ) = \overline{Y_i(1)-Y_i(0)}$
 ...
 - L'effet causal moyen est également appelé effet de traitement moyen.
...
## Estimations et questions causales

- Avant de discuter de la randomisation et de la façon dont elle nous permet d'estimer l'effet causal moyen, sachez que l'effet causal moyen est un type d'estimation.

- Une estimation est une quantité que vous souhaitez connaître (à partir de vos données). C'est la cible de votre recherche que *vous* avez défini.

- Être précis pour votre question de recherche, c'est être précis pour votre estimation. Pour les questions causales, il s'agit de préciser:
   - Le résultat
...
- Différences d'effet causal moyen conditionel: différences d'effet de traitement moyen pour un groupe par rapport à un autre groupe.
...
- Les estimations sont discutées en détail dans le module [estimations et estimateurs](estimation.html).
...
- Randomisation (du traitement): assigner des sujets avec une probabilité connue à des conditions expérimentales.
  ...
  - Mais la taille et d'autres caractéristiques de votre échantillon affecteront votre pouvoir et votre capacité à extrapoler votre découverte à d'autres populations.
...
- Nous pouvons donc écrire un estimateur pour l'effet de traitement moyen:

\centering
	$\hat{\overline{\tau_i}} = ( \overline{Y_i(1)} | T_i = 1 ) - ( \overline{Y_i(0)} | T_i = 0 )$

- En espérance (ou en moyenne sur des expériences répétées, $\hat{\overline{\tau_i}}$ égal l'effet de traitement moyen:
...
- Nous pouvons donc simplement prendre la différence de ces estimateurs sans biais $\overline{Y_i(1)}$ et
 $\overline{Y_i(0)}$ pour obtenir une estimation non biaisée de l'effet de traitement moyen.
...
```{r randomassignment, echo=FALSE, out.width='70%',fig.cap='Assignation aléatoire Random assignment de cet échantillon aléatoire des foyers'}
...
Pour faire des postulats de causalité avec une expérience (ou pour juger si nous croyons aux affirmations d'une étude), nous avons besoin de trois hypothèses de base:
 ...
 - l’hypothèse SUTVA (Stable unit treatment value assumption).
...
1. La non-interférence --  Le résultat potentiel d'un sujet reflète uniquement si ce sujet reçoit lui-même le traitement. Il n'est pas affecté par la façon dont les traitements sont attribués à d'autres sujets.
...
**Discutez en petits groupes :** Aidez-moi à concevoir le prochain projet pour répondre
...
## Discussion
 ...
 - Quels étaient les éléments clés, les forces et les faiblesses des études aléatoires?

 - Quels étaient les éléments clés, les forces et les faiblesses des études observationnelles?
