...
- La statistique de test pour cette hypothèse est presque toujours la simple différence de moyennes (c'est-à-dire `meanTT()` ci-dessus).
...
- Comment doit-on interpréter $p$=`r round(pMeanTT,2)`? Qu'en est-il de $p$=`r round(pMeanRankTT,2)` (nos tests pour l'hypothèse nulle forte)?

- Que signifie "rejeter" $H_0 : y_{i,1}=y_{i,2}$ à $\alpha=.05$ ?


## Taux de faux positifs dans les tests d'hypothèse {.allowframebreaks}
...
```{r plotecdf, results='asis', echo=FALSE, message=FALSE, warning=FALSE,fig.cap='Distributions des p-valeurs lorsqu'il n'y a aucun effet pour quatre tests avec n=10. Un test qui contrôle son taux de faux positifs doit avoir des points sur ou en dessous de la ligne diagonale.',out.width='.6\\textwidth'}
...
abline(0, 1, col = "gray")
legend("topleft",
  legend = c("Moindre carrés", "Neyman", "Diff des moyennes d'inférence de randomisation", "Diff des moyennes d'inférence de randomisation TODO Rand Inf Mean Diff Ranks"),
...
## Approfondir les tests d'hypothèses
  ...
  - Même si une procédure de test donnée contrôle le taux de faux positifs pour un seul test, elle peut ne pas contrôler le taux pour un groupe de tests.
    Voir [10 choses à savoir sur les comparaisons multiples](https://egap.org/methods-guides/10-things-you-need-know-about-multiple-comparisons)
    pour les approches de contrôle de taux de rejet dans plusieurs tests.
...
## Que savoir d'autre sur les tests d'hypothèse {.allowframebreaks}



  - Une estimation ponctuelle basée sur un test d'hypothèse est appelée une estimation ponctuelle de Hodges-Lehmann (@rosenbaum1993hlp,@hodges1963elb).

  - Un ensemble de tests d'hypothèse peut être combiné en un seul test d'hypothèse (@hansen:bowers:2008,@caughey2017nonparametric).
  ...
  - Étant donné qu'un test d'hypothèse est un modèle de résultats potentiels, on peut utiliser les tests d'hypothèse pour en savoir plus sur des modèles complexes, tels que des modèles de débordement et de propagation des effets de traitement à travers les réseaux (@bowers2013reasoning, @bowers2016research, @bowers2018models)
...
## Exercice : Tests d'hypothèse et statistiques de test
...
1. **Le taux d'erreur par famille (family wise error rate / FWER)** est de $P(V>0)$ (Probabilité de tous les faux positifs).
    ...
    - voir par exemple les projets de l'OES <http://oes.gsa.gov> : les agences fédérales choisissent quand elles détectent des résultats.
...
Pouvons-nous détecter un effet sur le résultat `Y1`? (c'est-à-dire, l'un des cinq tests d'hypothèse produit-il une $p$-valeur suffisamment petite?)
...
- La perte de pouvoir statistique induite par les approches d'ajustement nous incite à réfléchir aux *questions que nous voulons poser sur les données*.
