...
# l'estimande est la différence moyenne entre Y(Z=1) et Y(Z=0)
...
- Toutes les covariables doivent être pré-traitement. N'abandonnez pas d'observations pour cause de données manquantes.
    ...
    - Voir le module sur [les menaces à la validité interne](threats-to-internal-validity-of-randomized-experiments.html) et les [10 choses à savoir sur l'ajustement de covariables](https://egap.org/resource/10-things-to-know-about-covariate-adjustment/).
...
## Puissance statistique et design par grappe
- Voir le [module sur la randomisation](randomization.html).

- Etant donné un $N$ fixe, un design par grappe est légèrement moins puissant qu'un design sans grappe.
     - La différence est souvent substantielle.

- Il faut estimer correctement la variance :
     - Erreur standard robuste pour grappe (robust clustered standard error, RCSE)
...
## Le design par grappe dans la recherche observationelle
...
# confirmez que cela produit des données avec 10 dans chacune des 100 grappes
...
- Quelle est la plage de variation plausible de la variable de résultat ?
     ...
     - Un design avec un mouvement possible limité pour la variable de résultat peut ne pas être puissant.
