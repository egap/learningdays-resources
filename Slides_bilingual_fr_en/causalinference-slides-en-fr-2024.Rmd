...
# Causal effects \| *Les effets causals*
...
-   "$D$ cause $Y$" est une affirmation sur ce qui n'a pas eu lieu.
    ...
    -   "Si $D$ n'avait pas été donné, il n'y aurait pas de $Y$."
...
::: {.cols data-latex=""}
::: {.col data-latex="{0.48\\textwidth}"}
-   We must define what the treatment means and what it means to not
    have the treatment.
...
::: {.col data-latex="{0.48\\textwidth}"}
-   Nous devons définir la condition de traitement et de contrôle pour
...
## A random assignment produces an estimate \| *Une assignation aléatoire produit une estimation*

\centering

![](../Images/fig11-randomassignment1.png){width="320"}

## A different random assignment produces another estimate \| *Une autre assignation aléatoire génère une autre estimation*
...
| $\widehat{ATE}$ | 0   | 1/6 | 1/3 | 1/2 | 2/3 | 5/6 | 1   |
|-----------------|----:|----:|----:|----:|----:|----:|----:|
| Count           | 1   | 36  | 225 | 400 | 225 | 36  | 1   |

$$0*\frac{1}{924} + \frac{1}{6}*\frac{36}{924} + \frac{1}{3}*\frac{225}{924} + \frac{1}{2}*\frac{400}{924} + \frac{2}{3}*\frac{225}{924} + \frac{5}{6}*\frac{36}{924} + 1*\frac{1}{924} = \frac{1}{2}$$
...
::: {.cols data-latex=""}
::: {.col data-latex="{0.48\\textwidth}"}
2.  For the treated units, we observe $Y_i=Y_i(1)$.  

3.  For the control units, we observe $Y_i=Y_i(0)$.
:::
...
::: {.cols data-latex=""}
::: {.col data-latex="{0.48\\textwidth}"}
4.  Because the units were randomly assigned to treatment, the average of the $Y_i=Y_i(1)$ for the treated units is a good estimate of the average of $Y_i(1)$ for all the units. The same logic applies to the units randomly assigned to control.
:::
...
4.  Parce que les unités ont été assignées de manière aléatoire au
    traitement, la moyenne de $Y_i=Y_i(1)$ pour les unités traitées est une bonne estimation de la moyenne de
...
$$\widehat{ATE} = \overline{({Y_i}|D_i=1)} - \overline{({Y_i}|D_i=0)}$$

::: {.cols data-latex=""}
::: {.col data-latex="{0.48\\textwidth}"}
5.  So we plug in these estimates for $\overline{Y_i(1)}$ and $\overline{Y_i(0)}$ to estimate the ATE.
:::
...
::: {.col data-latex="{0.48\\textwidth}"}
5.  Donc nous utilisons ces estimations de $\overline{Y_i(1)}$ et $\overline{Y_i(0)}$ pour estimer l'ATE.
:::
:::
...
-   Nous ne pourrions pas obtenir pas exactement $\overline{Y_i(1)}$ d'une assignation
...
## Definition: Random assignment of treatment \| *Le définition: L'assignation aléatoire du traitement*
...
-   Careful: The condition assigned is not always the same as the
    condition received.
:::
...
-   Attention ! La condition assignée n'est pas toujours la même que la
    condition reçue.\
:::
:::
...
::: {.col data-latex="{0.48\\textwidth}"}
1.1 La non-interférence -- Le résultat potentiel d'un sujet reflète
...
::: {.cols data-latex=""}
::: {.col data-latex="{0.48\\textwidth}"}
-   Randomized studies have internal validity -- high confidence that we
    have learned the causal effect of a treatment on an outcome -- from randomization.
...
-   La randomisation apporte une validité interne élevée à une étude --
    nous avons appris l'effet causal du traitement sur le résultat avec
    un certain niveau de confiance -- grâce à l'assignation aléatoire.

-   Les études observationelles ont besoin d'autres hypothèses
    fortes pour pour tenter de traiter le biais de sélection.
:::
:::
...
::: {.col data-latex="{0.48\\textwidth}"}
-   Le résultat d'une étude spécifique à un endroit et moment
