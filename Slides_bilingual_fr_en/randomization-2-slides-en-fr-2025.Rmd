...
1.   Effet moyen de traitement conditionnel (CATE): L'effet moyen d'un des factors, en maintenant fixe la valeur de l'autre factor.
...
::: {.col data-latex="{0.48\\textwidth}"}
2.   Effet d'interaction : l'effet d’un facteur peut dépendre de la condition d'assignation de l'unité à un autre facteur. Cela signifie qu’un factor peut amplifier ou réduire l'effet de l'autre. 
:::
:::
...
-   Nous pouvons assigner de manière aléotoire l'**incitation** $D$ (l'encouragement) à suivre
...
-   So we can't learn the ATE of $D$ on $Y$ for all the units.

-   But we mayb be able to target other estimands.
:::
...
-   Alors nous ne pouvons pas apprendre l'ATE de $D$ sur $Y$ pour toutes les unités.
