...
library(estimatr)
library(styler)
### Para hacer mas rapido DeclareDesign (plan() en el chunk diagnosis2)
library(future)
...
## Puntos clave para la estimación I
  ...
  -  Un efecto causal, $\tau_i$, es una comparación de resultados potenciales no observados para cada unidad  $i$, por ejemplo:  $\tau_{i} = Y_{i}(T_{i}=1) -  Y_{i}(T_{i}=0)$ o  $\tau_{i} = \frac{Y_{i}(T_{i}=1)}{ Y_{i}(T_{i}=0)}$.

  - Para aprender sobre $\tau_{i}$, podemos tratar a  $\tau_{i}$ como un **estimando** o una cantidad objetivo a ser estimada (discutido acá),  o como una cantidad objetivo sobre la cual se plantearán hipótesis (sesión de pruebas de hipótesis).

 - Hay muchas personas que se enfocan en el  **estimando**  del **efecto promedio del tratamiento**  (average treatment effect, ATE), $\bar{\tau}=\frac{1}{n}\sum_{i=1}^n\tau_{i}$, en parte, porque permite una  **estimación** fácil.
...
La clave para la estimación en la inferencia causal es elegir un estimando que permita aprender sobre alguna pregunta teórica o de políticas públicas. Para esto, el ATE es una opción, pero otros estimandos comunes también incluyen el ITT, LATE/CACE, ATT o ATE para algún subgrupo (o incluso una diferencia de un efecto causal entre grupos).
  ...
  - Un **estimador** es una fórmula para hacer una estimación (o cálculo de un número o intervalo) sobre el valor de un estimando. Por ejemplo, la diferencia de medias observadas para $m$ unidades tratadas es un estimador de $\bar{\tau}$:
...
## Puntos clave sobre la estimación V
 ...
 -  **Analizar conforme se aleatoriza** en cuanto a la estimación
 significa que (1) nuestros errores estándar deben medir la variabilidad de la
 aleatorización y (2) el objetivo de nuestros estimadores debe ser
...
provenientes de experimentos aleatorios aunque sí usamos **ajuste de
covarianza** (que son cálculos idénticos). Las covariables pueden hacer que nuestra
...
aleatorios aunque, algunos se usa el mismo comando de `lm(Y~Z+x)` en R.

- Ajuste de covarianza simple produce un estimador sesgado
  [@lin_agnostic_2013]. Pero el sesgo se hace pequeño en cuanto el N se hace
  grande.
...
Supongamos que estamos interesados en el ATE, o $\bar{\tau} = \frac{1}{n}\sum_{i = 1}^n \tau_{i}$. ¿Cuál sería un buen estimador?
...
Dos candidatos:
 ...
 2. Una diferencia de medias después de recodificar el valor máximo de las observaciones $Y_i$ (una
    especie de media "truncada" (winsorized), con lo que se busca evitar que los valores extremos ejerzan demasiada
...
## Ordenamos los estimadores por el promedio de sus estimaciones y mostramos
## una submuestra de estimaciones como puntos: cada punto se puede leer como
...
sim_plot <- ggplot(sims1_plot, aes(x = estimate, y = estimator)) +
  ...
  annotate("text",
    ...
    label = paste0("ATE verdadero = ", round(trueATE, 1))
  ) +
  scale_y_discrete(expand = expansion(add = c(0.5, 1.2))) +
  labs(
    x = "Estimación", y = "",
    caption = "Cada punto gris es la estimación de un experimento posible (100 de las 10,000).\nEl punto negro marca el promedio de las estimaciones de cada estimador."
  ) +
  ## Caption centrada para no chocar ni con el logo de EGAP (esquina
  ## inferior izquierda) ni con el número de página (esquina derecha).
  theme(
...
## La línea marca el estimando del diseño con bloques tal como quedó
## registrado en las simulaciones (es el mismo valor en las 10,000).
trueATEb <- unique(sims2$estimand)
## Ordenamos los estimadores por el promedio de sus estimaciones para que
## el estimador sesgado se separe de los demás en la gráfica.
sims2_plot <- sims2 %>% mutate(estimator = reorder(estimator, estimate, FUN = mean))

## Mostramos una submuestra de las estimaciones como puntos para que cada
## punto se pueda leer como "un experimento posible". Con las 10,000 los
## puntos taparían las cajas. Cada chunk de simulación fija su propia
## semilla, así que esta semilla local no cambia resultados posteriores.
set.seed(20260728)
...
sim_plot2 <- ggplot(sims2_plot, aes(x = estimate, y = estimator)) +
  ...
  annotate("text",
    ...
    label = paste0("ATE verdadero = ", round(trueATEb, 1))
  ) +
  ## Espacio adicional arriba de la primera caja para la etiqueta del ATE
  scale_y_discrete(expand = expansion(add = c(0.6, 1.4))) +
  labs(
    x = "Estimación", y = "",
    caption = "Cada punto gris es la estimación de un experimento posible (100 de las 10,000); el punto negro marca el promedio."
  ) +
  theme(
    text = element_text(size = 20),
...
Supongamos que tenemos datos provenientes de 10 conglomerados con 100 personas (en 2 conglomerados) o 10 personas por grupo (en 8 conglomerados). El tamaño total de los datos es `r nrow (dat1)`.
...
```{r showresclus1}
kableExtra::kable(res_clus[, c(1, 5:7)], digits = 2, booktabs = TRUE, linesep = "", caption = "Desempeño de estimadores y pruebas con 10000 simulaciones del diseño aleatorizado por conglomerados para diferentes estimadores e intervalos de confianza")
```
...
```{r showresclus2}
kableExtra::kable(res_clus[, c(1:3)], digits = 3, booktabs = TRUE, linesep = "", caption = "Desempeño de estimadores y pruebas con 10000 simulaciones del diseño aleatorizado por conglomerados para diferentes estimadores e intervalos de confianza")
```
...
## El estimador de Lin (Y~Z*I(clus_size-mean(clus_size))) a veces no produce
## una estimación; quitamos esos NA igual que en la tabla anterior.
## Acortamos las etiquetas con los mismos nombres que usa la tabla para que
## el eje ocupe menos espacio y la gráfica y la tabla se lean juntas.
...
sim_plot3 <- ggplot(sim_clus_plot, aes(x = estimate, y = estimator)) +
  ...
  annotate("text",
    ...
    label = paste0("ATE verdadero = ", round(trueclusATE, 2))
  ) +
...
Recuerden que $\bar{y}_1$ es la *proporción* de $y_{1}=1$ en los datos.

@freedman2008randomization  nos muestra que el estimador del coeficiente logit ajustado por covariables es un estimador sesgado del estimando de la diferencia en los log-odds. Así mismo también presenta un estimador consistente para ese mismo estimando.
...
¿Cómo debemos interpretar las siguientes estimaciones? (¿Cuáles son los supuestos que el estimador de diferencia de
medias requiere? ¿Cuáles son los supuestos que el
...
- La logística con solo `Z` está *saturada*: su coeficiente es exactamente la diferencia de log-odds observada, $\text{logit}(\bar{Y}_{Z=1}) - \text{logit}(\bar{Y}_{Z=0})$. Es un estimador consistente de la diferencia marginal $\delta$.

- Al agregar una covariable cambia el blanco: el coeficiente de `Z` en `glm(Y ~ Z + x1)` apunta a la diferencia de log-odds *manteniendo fija* `x1` --- una cantidad condicional.

- Los odds ratios no son *colapsables*: la diferencia condicional de log-odds queda más lejos de cero que la marginal, aun sin confusión. Por eso el coeficiente ajustado es un estimador sesgado de $\delta$, y esa brecha no se achica al crecer $N$ [@freedman2008randomization].
...
Los dos estimadores del mismo estimando de log-odds, con el mismo modelo ajustado (`Y ~ Z + x1`):
...
## El tmle produjo algunas estimaciones infinitas; las quitamos igual que
...
```{r showresbin1}
kableExtra::kable(res_bin[, c(1, 2, 3, 4)], digits = 3, booktabs = TRUE, linesep = "", caption = "Desempeño de estimadores y pruebas con 1000 simulaciones del diseño aleatorizado para diferentes estimadores.")
```
...
## Una simulación más liviana que el diagnóstico anterior: solo los tres
## estimadores basados en logit del estimando de log-odds, para varios N.
...
etiquetas_bias <- c(
  adj = "Coeficiente logit ajustado por x1",
  plugin = "Plug-in de Freedman (mismo modelo)",
  unadj = "Logit sin covariables (saturado)"
)
bias_res$estimator <- factor(bias_res$estimator,
  levels = names(etiquetas_bias), labels = etiquetas_bias
)
bias_plot <- ggplot(bias_res, aes(x = N, y = bias, color = estimator, shape = estimator)) +
  ...
  ## La leyenda va DENTRO del panel (zona vacía a la derecha) para que la
  ## figura quepa completa en la diapositiva
  theme(
...
## En experimentos las covariables sirven para la precisión --- pero "controlar por" no es gratis

- La aleatorización equilibra las covariables en promedio: no necesitamos `x` para identificar el ATE. Aun así, una covariable que predice el resultado puede achicar nuestros errores estándar --- por eso ajustamos.

- El reflejo de los estudios observacionales --- reportar el coeficiente de `Z` en `lm(Y ~ Z + x)` --- es otro estimador, y @freedman2008rae mostró que es un estimador **sesgado** del ATE. El sesgo disminuye cuando $N$ crece [@lin_agnostic_2013], así que la pregunta es cuándo importa y qué hacer al respecto.

- Plan: construir un experimento pequeño donde el sesgo se vea, comparar cuatro estimadores y luego observar cómo el sesgo se desvanece a medida que el experimento crece.

## Un experimento pequeño con una covariable asimétrica que predice el resultado

```{r covadjpop, echo=TRUE}
set.seed(5)
N3 <- 20 # el tamaño de un piloto o de un análisis a nivel de conglomerado
## Una covariable asimétrica: la mayoría de las unidades son pequeñas,
## unas pocas quedan lejos en la cola
x <- exp(rnorm(N3, mean = 0, sd = 1))
## x predice fuertemente el resultado bajo control, así que
## el ajuste promete una ganancia real de precisión
y0 <- x + rnorm(N3, sd = 0.5)
## Los efectos son más grandes justo donde x es extrema. El sesgo de
## Freedman crece con esta covarianza entre apalancamiento
## (leverage) y tamaño del efecto
y1 <- y0 + 0.75 * x^2
...
Asignaremos 10 de las 20 unidades al tratamiento:
...
## Tres estimadores conocidos del mismo ATE
...
## La reparación de Lin a la crítica de eficiencia de Freedman: centrar
## x, interactuar con Z y usar errores estándar robustos
...
## lm_lin es el principio del plug-in aplicado al modelo lineal

La misma movida que el plug-in de Freedman para el logit de arriba: ajustamos el modelo, predecimos el resultado de cada unidad bajo $Z=1$ y bajo $Z=0$, promediamos y tomamos la diferencia. Para un modelo lineal con interacciones, el plug-in **es** `lm_lin` --- dos nombres, un solo estimador.
...
(Sin la interacción, el plug-in de `lm(Y ~ Z + x)` devuelve simplemente el coeficiente de `Z`, así que el estimador ingenuo no gana nada con esta movida.)

## El sesgo restante se puede estimar con los datos observados --- y luego restar

@chang2024exact escriben el sesgo exacto del estimador de Lin --- covarianzas entre el *apalancamiento* (leverage) de cada unidad y su resultado --- y cada término se puede estimar sin sesgo, así que restar esa estimación produce un estimador exactamente insesgado para cualquier $N$. Su Teorema 4.2 para una covariable:

```{r covadjcma, echo=TRUE}
cma_bias_hat <- function(Y, Z, x) {
  n <- length(Y)
  xs <- (x - mean(x)) / sqrt(mean((x - mean(x))^2)) # estandarizar x
  h <- xs^2 # el apalancamiento de cada unidad
  zt <- Z == 1
  zc <- Z == 0
  nT <- sum(zt)
  nC <- sum(zc)
  xbT <- mean(xs[zt])
  xbC <- mean(xs[zc])
  LT <- mean(xs[zt]^2) - xbT^2 # varianzas de x por grupo ...
  LC <- mean(xs[zc]^2) - xbC^2
  NT <- mean(Y[zt] * xs[zt]) - mean(Y[zt]) * xbT # ... covarianzas de x, Y
  NC <- mean(Y[zc] * xs[zc]) - mean(Y[zc]) * xbC
  covhT <- mean(h[zt] * Y[zt]) - mean(h[zt]) * mean(Y[zt]) # apalancamiento, Y
...
## El cuarto estimador: lm_lin menos su sesgo estimado
...
cma_est_fun <- function(data) {
  ...
  ## Conservamos el error estándar del estimador sin corregir: la
  ## corrección no cambia la distribución de muestras grandes (Chang et
  ## al. 2024). Observen qué le hace esto a la cobertura con N = 20
...
Luego, como antes, reasignamos el tratamiento 10,000 veces y aplicamos los cuatro estimadores a cada experimento simulado.
...
## Resumen numérico que usan el gráfico, la tabla y el texto de abajo
covsum <- simsc %>%
  group_by(estimator) %>%
  summarize(
    bias = mean(estimate - estimand),
...
## El mismo formato de las secciones anteriores: cada punto gris es un
## experimento posible; las cajas resumen las 10,000; el punto negro es
## el promedio
...
covadj_plot <- ggplot(simsc_plot, aes(x = estimate, y = estimator)) +
  ...
  annotate("text",
    ...
    label = paste0("ATE verdadero = ", round(trueATE3, 2))
  ) +
...
## ¿Cuál estimador está más cerca del valor real?
...
covsum %>%
  ...
  kableExtra::kable(
    digits = 2,
    col.names = c("Estimador", "Sesgo", "Desviación estándar", "RMSE", "Cobertura")
  )
```

- `Y~Z+x` subestima el ATE de `r round(trueATE3, 2)` por `r round(abs(naive_row$bias), 2)` --- cerca del `r round(100 * abs(naive_row$bias) / trueATE3)`% --- y sus intervalos cubren solo el `r round(100 * naive_row$coverage)`% de las veces.

- `lm_lin` tiene el RMSE más pequeño; su sesgo es cerca de un tercio del sesgo ingenuo, pero no es cero.

- La corrección exacta elimina el sesgo --- y lo paga con varianza. Su error estándar prestado no sabe nada de ese ruido extra, así que sus intervalos cubren solo el `r round(100 * cma_row$coverage)`%. La insesgadez es una propiedad entre varias, no todo el juego.

- Hasta la diferencia de medias insesgada cubre el `r round(100 * dim_row$coverage)`%, no el 95%: $N=20$ con un resultado asimétrico exige demasiado a todos los intervalos basados en la aproximación Normal.

## El sesgo del ajuste se desvanece cuando el experimento crece

Copias de las mismas 20 unidades, así que solo cambia $N$: duplicar el experimento reduce el sesgo ingenuo aproximadamente a la mitad. A diferencia del coeficiente logit ajustado de arriba, este es un problema de muestras pequeñas, no de estimando equivocado.

```{r covadjbiasn, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE}
## Una simulación más liviana: copias de las mismas 20 unidades, de modo
## que los momentos de la población quedan fijos y solo cambia N.
## 2,000 reasignaciones por cada N.
...
biasn_labels <- c(
  naive = "Y~Z+x ('controlar por')",
  lin = "lm_lin",
  debiased = "lm_lin - sesgo estimado",
  dim = "Diferencia de medias"
)
...
## Consejos sobre el ajuste de covarianza

- En experimentos medianos y grandes: ajusten con unas pocas covariables **preespecificadas** que predigan el resultado, usando `lm_lin()` --- o, de manera idéntica, el plug-in con un modelo lineal interactuado [@lin_agnostic_2013]. Ganan precisión; el sesgo restante es de orden $1/N$.

- En experimentos pequeños (decenas de unidades): el sesgo puede ser una fracción visible del efecto y puede distorsionar la cobertura. Reporten la diferencia de medias sin ajustar junto a cualquier estimación ajustada, y prefieran usar las covariables en el **diseño** (bloques) y no en el análisis.

- Existe una corrección exactamente insesgada [@chang2024exact]. Cambia sesgo por varianza: con $N=20$ el RMSE siguió favoreciendo a `lm_lin`. Elijan estimadores por sus propiedades --- sesgo, precisión, cobertura --- simulando su propio diseño, como en toda esta sesión.

- Elijan las covariables antes de ver los resultados. Ajustar después de mirar invita a la pesca de resultados (más sobre esto en la sesión de transparencia).
...
- (Conectando los conceptos de estimación y pruebas) Los errores estándar de los estimadores nos permiten calcular intervalos de confianza y valores $p$. Ciertos estimadores tienen
...
cuando se visita o no es un atributo fijo, como un resultado potencial).

 - $Y$: variable de resultado ($y_ {i, Z}$ o $y_ {i, Z_i = 1}$ para el resultado potencial de
 ...
 - $Z$: asignación del tratamiento ($Z_i = 1$ si se asigna a una visita, $Z_i = 0$ si no se
   asigna a una visita)
 - $D$: tratamiento recibido ($D_i = 1$ si la persona $i$ abre la puerta, $D_i = 0$ si
   decide no abrir la puerta) (usando $D$ aquí porque $D_i = d_ {i, 1} Z_ {i} + d_ {i, 0} (1-Z_i)$)
...
\begin{tabular}{llcc}
		       ...
		       &       & $D=0$ & $D=1$ \\
		       \midrule
\multirow{2}{*}{$Z=0$} & $D=0$ & los que nunca lo toman & los que cumplen \\
...
aquellos que abrieron la puerta (los que cumplen, los que siempre lo toman y los que desafían). Utilizamos
...
diferentes grupos (pueden imaginar que los sustituyen y luego los reorganizan para que podamos
...
ITT, no es necesario que consideremos todos los tipos de sujetos que vimos previamente. No tenemos sujetos que desafían
...
Primero podemos aprender sobre el efecto de la política en sí. Para calcular el
ITT, no es necesario que consideremos todos los tipos de sujetos que vimos previamente. En este caso, no tenemos sujetos que desafían
($p_D = 0$) y sabemos que el ITT tanto para los que siempre toman como para los que nunca toman es 0.
...
\begin{equation}
( (\bar{y}_{1}|A)- (\bar{y}_{0}|A))p_A = 0  \text{ y } ( (\bar{y}_1|N) - (\bar{y}_{0}|N) )p_N = 0
\end{equation}
...
Pero esta comparación está asociada con $x$: una simple comparación $\bar{Y} | D = 1 - \bar {Y} | D = 0$ nos deja ver las diferencias en la variable de resultado ocasionadas por $x$ además de
la diferencia causada por $D$. (Los números que se muestran a continuación están basados en una simulación)

\begin{center}
\begin{tikzcd}[ampersand replacement=\&]
Z  \arrow[from=1-1,to=1-2] \arrow[from=1-1, to=1-4, bend left, "\text{0 (exclusión)}"] \& D  \arrow[from=1-2,to=1-4] \& \& y \\
(x_1 \ldots x_p) \arrow[from=2-1,to=1-1, "\text{-.006 (como si fuera al azar)}"]  \arrow[from=2-1,to=1-2, ".06"] \arrow[from=2-1,to=1-4, ".48"]
\end{tikzcd}
\end{center}
...
Reuniendo todo:^[funciona bien cuando $Z \rightarrow D$ no es débil. Vean @imbens2005robust para una historia con moraleja al respecto]
