...
- "Power" is an operating characteristic of hypothesis tests (and by extension, confidence intervals). Estimators do not have statistical power.

## Starting point for power analysis

- Power analysis is something we do *before* we run a study.

  - Helps you figure out **how many units** you need to detect a given effect size.
...
Power analysis shows us how much the probability of getting a significant result changes when we change our assumptions.

- Many disciplines have settled on a target power value of 0.80. Reasons to move to 0.90?
- For example: if our current design has power of 0.70, what would it take to get to 0.80? More units? A less noisy outcome? A stronger treatment?
- We can tweak our designs and assumptions until we can be confident that our experiment will return statistically significant results "most" of the time (where "most" = 80% or 90%)
...
# What raises or lowers power?

## Power is increasing in $N$ {.allowframebreaks}

```{r powplot_by_n, echo=TRUE}
## With sd = 1, delta is in standard-deviation units of the outcome:
## delta = 0.25 means an effect of 0.25 standard deviations. That
## standardized effect is Cohen's d: d = (m_1 - m_2) / sigma.
...
```{r powsum, echo=TRUE}
# for each estimator, power = proportion of simulations with p.value < 0.05
...
interval is the complement of the false positive rate of a hypothesis test: 95%
coverage corresponds to a 5% false positive rate.)
