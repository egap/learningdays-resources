...
- So, if a counterfactual causal effect of the treatment, $Z$, for Jake occurs when  $y_{\text{Jake},Z=1} \ne y_{\text{Jake},Z=0}$, then how can we learn about the causal  effect?
...
- The hypothesis testing approach to causal inference doesn't *provide* a best guess but instead tells you about evidence or information about a best guess.

- The estimation approach provides a best guess but doesn't tell you how much you know about that guess.

- These approaches can converge and we nearly always report both: "Our best guess of the treatment effect was 5, and we could reject the idea that the effect was 0 ($p$=.01)."
...
```{r}
kable(dat, col.names = c("Outcome", "Treatment", "yi0", "ITE", "yi1", "Y>0"))
```
...
possibleMeanDiffsH0 <- replicate(10000, 
                                 meanTZ(ys = Y, z = repeatExperiment(N = 10)))
set.seed(123456)
possibleMeanRankDiffsH0 <- replicate(10000, 
                            meanrankTZ(ys = Y, z = repeatExperiment(N = 10)))
```
...
library(coin)
set.seed(12345)
pMean2 <- pvalue(oneway_test(Y ~ factor(Z), data = dat, 
                             distribution = approximate(nresample = 1000)))
dat$rankY <- rank(dat$Y)
pMeanRank2 <- pvalue(oneway_test(rankY ~ factor(Z), data = dat, 
                                 distribution = approximate(nresample = 1000)))
pMean2
pMeanRank2
```
...
```{r installritools,echo=TRUE, results='hide',warnings=FALSE,cache=FALSE}
dev_mode() ## dont install the package globally
install_github("markmfredrickson/RItools@randomization-distribution", force = TRUE)
dev_mode()
```

Then use the `RItest` function.

```{r useritools, echo=TRUE,cache=FALSE}
dev_mode()
library(RItools)
...
- Power of hypothesis tests ([Module on Statistical Power and Design Diagnosands](https://egap.github.io/learningdays-guide/statistical-power-and-design-diagnosands.html)).
...
- The test statistic for this hypothesis is nearly always the difference of means (i.e., `meanTZ()` above).
...
```{r fig.cap="Boxplot of observed outcomes by treatment status", results='asis'}
...
c(
  observedTestStat = observedMeanTZ, 
  stderror = seEstATE, 
  tstat = obsTStat,
  pval = 2 * min(
...
- How should we interpret $p$=`r  pMeanTZ`? What about $p$=`r pMeanRankTZ`?
...
Compare tests by plotting the proportion of p-values less than any given number. The randomization inference tests control the false positive rate.
...
pDistBigLst <- mclapply(1:1000, function(i) {
  collectPValues(y = bigdat$Ybin, z = bigdat$Z, thedistribution = approximate(nresample = 1000))
}, mc.cores = 8)
pDistBig <- simplify2array(pDistBigLst)
```

```{r plotecdfBig, results='asis', echo=FALSE, message=FALSE, warning=FALSE,out.width='.6\\textwidth',fig.cap="P-value distributions when there are no effects for four tests with n=60 and a binary outcome. A test that controls its false positive rate should have points on or below the diagonal line."}
par(mfrow = c(1, 1), mgp = c(1.25, .5, 0), oma = rep(0, 4), mar = c(3, 3, 0, 0))
plot(c(0, 1), c(0, 1),
  type = "n",
  xlab = "p-value=p", ylab = "Proportion p-values < p"
...
pDistBigLst2 <- mclapply(1:1000, function(i) {
  collectPValues(y = bigdat$Y, z = bigdat$Z, thedistribution = approximate(nresample = 1000))
}, mc.cores = 8)
pDistBig2 <- simplify2array(pDistBigLst2)
```

```{r plotecdfBig2, results='asis', echo=FALSE, message=FALSE, warning=FALSE, out.width='.6\\textwidth',fig.cap='P-value distributions when there are no effects for four tests with n=60 and a continuous outcome. A test that controls its false positive rate should have points on or below the diagonal line.'}
par(mfrow = c(1, 1), mgp = c(1.25, .5, 0), oma = rep(0, 4), mar = c(3, 3, 0, 0))
plot(c(0, 1), c(0, 1),
  type = "n",
  xlab = "p-value=p", ylab = "Proportion p-values < p"
...
Notice:
 ...
 - The $p$-value is how rare it would be to see the observed test statistic (or a value farther away from the hypothesized value, like 0) in the world of the null.
...
## False positive and false negative errors {.allowframebreaks}
...
- A single test of a single hypothesis should encourage false positive errors rarely (for example, if we set $\alpha=.05$) then we are saying that we are comfortable with our testing procedure making false positive errors **no more than 5% of tests of a given treatment assignment in a given experiment**.
...
## False Positive Rates in Multiple Hypothesis Testing {.allowframebreaks}
...
+---------------------------------------------+--------------------------+----------------------+-----------+
| Not true null hypotheses ($H_{true} \ne 0$) |             T            |           S          | $m - m_0$ |
+---------------------------------------------+--------------------------+----------------------+-----------+
|                                 Total       |            m-R           |           R          |    $m$    |
+---------------------------------------------+--------------------------+----------------------+-----------+

**Number of errors committed when testing $m$ null hypotheses** [@benjamini1995
's Table 1]. Cells are numbers of tests.  For example, $R$ are # of
"discoveries" and $V$ are false discoveries, $U$ are # of correct
non-rejections, and $S$ are # of correct rejections.


*Two main error rates to control when testing many hypotheses:*

 - **Family wise error rate (FWER)** is $P(V>0)$ (Probability of any false
       positive error) We'd like to control this if we plan to make a decision
...
- **False Discovery Rate (FDR)** is  $E(V/R | R>0)$ (Average proportion of
       false positive errors given some rejections.) We'd like to control this
       if we are using *this* experiment to plan *the next* experiment. We are
       willing to accept a higher probability of error in the interests of
       giving us more possibilities for discovery. For example, one could
...
What is the effect of one treatment on multiple outcomes? On which outcomes (out of many) did the treatment have an effect? Especially the second question can lead to the kind of uncontrolled family wise error rate problems that we referred to above.

Imagine we had five outcomes and one treatment:
...
theassign <- declare_assignment(m = 50)
po_1 <- declare_potential_outcomes(Y1_Z_0 = y0_1, Y1_Z_1 = y0_1 + tau1)
po_2 <- declare_potential_outcomes(Y2_Z_0 = y0_2, Y2_Z_1 = y0_2 + tau2)
po_3 <- declare_potential_outcomes(Y3_Z_0 = y0_3, Y3_Z_1 = y0_3 + tau3)
po_4 <- declare_potential_outcomes(Y4_Z_0 = y0_4, Y4_Z_1 = y0_4 + tau4)
po_5 <- declare_potential_outcomes(Y5_Z_0 = y0_5, Y5_Z_1 = y0_5 + tau5)
reveal_1 <- declare_reveal(Y1, Z)
reveal_2 <- declare_reveal(Y2, Z)
reveal_3 <- declare_reveal(Y3, Z)
reveal_4 <- declare_reveal(Y4, Z)
reveal_5 <- declare_reveal(Y5, Z)
...
## Multiple hypothesis testing: Multiple Outcomes {.allowframebreaks}

We could ask:

 - Can we detect an effect on outcome `Y1`? (i.e., does the hypothesis test produce a small enough $p$-value?)
```{r p1, echo=TRUE}
pvalue(oneway_test(Y1 ~ factor(Z), data = dat1))
## Notice that the t-test p-value is also a chi-squared test
## p-value. So the omnibus test is just a generalized t-test
pvalue(independence_test(Y1 ~ factor(Z),
  data = dat1,
  teststat = "quadratic"
))
```
...
```{r pmult, echo=TRUE}
p1 <- pvalue(oneway_test(Y1 ~ factor(Z), data = dat1))
p2 <- pvalue(oneway_test(Y2 ~ factor(Z), data = dat1))
p3 <- pvalue(oneway_test(Y3 ~ factor(Z), data = dat1))
p4 <- pvalue(oneway_test(Y4 ~ factor(Z), data = dat1))
p5 <- pvalue(oneway_test(Y5 ~ factor(Z), data = dat1))
...
```{r omnibus, echo=TRUE}
pvalue(independence_test(Y1 + Y2 + Y3 + Y4 + Y5 ~ factor(Z),
...
- We will (1) set the true causal effects to be 0, (2) repeatedly re-assign treatment and (3) do
each of those three tests each time.
...
ttest_Y1fn <- function(data) {
  p1 <- pvalue(oneway_test(Y1 ~ factor(Z), data = data))
  return(data.frame(statistic = NA, p.value = p1))
}
ttest_multfn <- function(data) {
  p1 <- pvalue(oneway_test(Y1 ~ factor(Z), data = data))
  p2 <- pvalue(oneway_test(Y2 ~ factor(Z), data = data))
  p3 <- pvalue(oneway_test(Y3 ~ factor(Z), data = data))
  p4 <- pvalue(oneway_test(Y4 ~ factor(Z), data = data))
  p5 <- pvalue(oneway_test(Y5 ~ factor(Z), data = data))
  theps <- c(p1, p2, p3, p4, p5)
...
ttest_omnibusfn <- function(data) {
  thep <- pvalue(independence_test(Y1 + Y2 + Y3 + Y4 + Y5 ~ factor(Z), data = data, teststat = "quadratic"))
...
recall that there are no causal effects at all for any of these outcomes.
 ...
 - A test of a single outcome has $p < .05$ no more than 5% of the simulations.

 - The omnibus test also shows a well-controlled error rate.

 - And we also show that using a multiple testing correction (here we use the
   "Holm" correction) also correctly controls the false positive rate.
...
theassign_mult <- declare_assignment(
  num_arms = 5,
  conditions = c("1", "2", "3", "4", "5"), assignment_variable = Z
)
po_mult <- declare_potential_outcomes(Y ~ y0_1 * (Z == "1") + y0_2 * (Z == "2") +
  y0_3 * (Z == "3") + y0_4 * (Z == "4") + y0_5 * (Z == "5"),
conditions = c("1", "2", "3", "4", "5"),
assignment_variables = Z
)
reveal_mult <- declare_reveal(assignment_variables = Z)
des2 <- thepop + theassign_mult + po_mult + reveal_mult
dat2 <- draw_data(des2)
options(digits=2)
## Z is treatment arm: 1,2,3,4,5
head(dat2[,-c(2:6,8)])
options(digits=4)
```
...
pair_tests1 <- pairwisePermutationTest(Y ~ Z, data = dat2, distribution = asymptotic(), method = "holm", teststat = "quadratic")
pair_tests1
```
## Multiple hypothesis testing: Multiple Treatment Arms

Here is an illustration of four different approaches to testing hypotheses with
multiple arms:

  -  do all of the pairwise tests and choose the best one (a bad idea);
  -  do all the pairwise tests and choose the best one after adjusting
the p-values for multiple testing (a fine idea but one with very low
statistical power);
  - test the hypothesis of no relationship between any arm
and the outcome (a fine idea);
  -  choose one arm to focus on in advance (a fine idea).

```{r}
ttest_Z1_vs_allfn <- function(data) {
  p1 <- pvalue(oneway_test(Y ~ factor(Z == "1"), data = data))
  return(data.frame(statistic = NA, p.value = p1))
}
overall_Zfn <- function(data) {
  itest <- independence_test(Y ~ Z, data = data)
  return(data.frame(statistic = NA, p.value = pvalue(itest)))
}
pairwise_testsfn <- function(data) {
  pair_tests <- pairwisePermutationTest(Y ~ Z, data = data, distribution = asymptotic(), method = "holm", teststat = "quadratic")
...
## dat2$ZF <- factor(dat2$Z)
## blah <- independence_test(Y~ZF,data=dat2,xtrafo = mcp_trafo(ZF = "Tukey")) #,teststat="quadratic")
## pair_tests2 <- pvalue(blah,method="unadjusted")
## pair_tests2
## thecontrasts <- rbind("2 - 1 " = c(1,-1,0,0,0,0),
## "3-1"=c(1,0,-1,0,0,0))
## blah2<- independence_test(Y~ZF,data=dat2,xtrafo = mcp_trafo(ZF = thecontrasts))
## pvalue(blah2,method="single-step")
##

ttest_Z1_vs_all <- declare_test(handler = label_test(ttest_Z1_vs_allfn), label = "t-test Z1 vs all")
overall_Z <- declare_test(handler = label_test(overall_Zfn), label = "Overall test")
...
des2_plus <- des2 + ttest_Z1_vs_all + overall_Z + pairwise_test + pairwise_test_adj
thediagnosands <- declare_diagnosands(fwer = mean(p.value < .05))
