...
library(coin)
set.seed(12345)
pMean2 <- pvalue(oneway_test(Y ~ factor(T), data = dat, 
                             distribution = approximate(nresample = 1000)))
dat$rankY <- rank(dat$Y)
pMeanRank2 <- pvalue(oneway_test(rankY ~ factor(T), data = dat, 
                                 distribution = approximate(nresample = 1000)))
pMean2
pMeanRank2
```
...
- The test statistic for this hypothesis is nearly always the difference of means (i.e., `meanTT()` above).
...
- How should we interpret $p$=`r  pMeanTT`? What about $p$=`r pMeanRankTT`?
...
```{r pmult, echo=TRUE}
p1 <- pvalue(oneway_test(Y1 ~ factor(T), data = dat1))
p2 <- pvalue(oneway_test(Y2 ~ factor(T), data = dat1))
p3 <- pvalue(oneway_test(Y3 ~ factor(T), data = dat1))
p4 <- pvalue(oneway_test(Y4 ~ factor(T), data = dat1))
p5 <- pvalue(oneway_test(Y5 ~ factor(T), data = dat1))
...
```{r omnibus, echo=TRUE}
pvalue(independence_test(Y1 + Y2 + Y3 + Y4 + Y5 ~ factor(T),
...
ttest_Y1fn <- function(data) {
  p1 <- pvalue(oneway_test(Y1 ~ factor(T), data = data))
  return(data.frame(statistic = NA, p.value = p1))
}
ttest_multfn <- function(data) {
  p1 <- pvalue(oneway_test(Y1 ~ factor(T), data = data))
  p2 <- pvalue(oneway_test(Y2 ~ factor(T), data = data))
  p3 <- pvalue(oneway_test(Y3 ~ factor(T), data = data))
  p4 <- pvalue(oneway_test(Y4 ~ factor(T), data = data))
  p5 <- pvalue(oneway_test(Y5 ~ factor(T), data = data))
  theps <- c(p1, p2, p3, p4, p5)
...
ttest_omnibusfn <- function(data) {
  thep <- pvalue(independence_test(Y1 + Y2 + Y3 + Y4 + Y5 ~ factor(T), data = data, teststat = "quadratic"))
...
ttest_T1_vs_allfn <- function(data) {
  p1 <- pvalue(oneway_test(Y ~ factor(T == "1"), data = data))
  return(data.frame(statistic = NA, p.value = p1))
}
