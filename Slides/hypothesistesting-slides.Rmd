...
collectPValues <- function(y, z, thedistribution = exact()) {
  ...
  thedata <- data.frame(newz = newz, y = y)
  thedata$ranky <- rank(y)
  thedata$newzF <- factor(thedata$newz)
  ## The four tests
  thelm <- lm(y ~ newz, data = thedata)
  ttestP2 <- difference_in_means(y ~ newz, data = thedata)
  owP <- pvalue(oneway_test(y ~ newzF,
    data = thedata,
    distribution = thedistribution
  ))
  owRankP <- pvalue(oneway_test(ranky ~ newzF,
    data = thedata,
    distribution = thedistribution
  ))
  ## Return the p-values
