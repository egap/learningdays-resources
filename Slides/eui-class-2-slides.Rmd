...
## Plan for the Day
 ...
 - Review of discussion from yesterday (Advantages, Disadvantages)
    - "Humans generalize, experiments do not"
    - "History matters" (so representativeness of samples is always limited)
    - Causal mechanisms require contexts.
    - Randomization breaks confounds, but does not balance every covariate every time.
 - Questions about experiments and their role in social science and policy arising after class?
 - How might hypothesis tests help us infer counterfactual causal effects in randomized experiments?
 - How might estimators help us infer counterfactual causal effects in randomized experiments?

# Hypothesizing about partially observed potential outcomes and summarizing evidence against those hypotheses

## General Approaches To Learning About The Unobserved Using Data

\centering
  \includegraphics[width=.6\textwidth]{Images/MorrowPlots.jpg}

## Two Approaches To Causal Inference: Potential Outcomes
...
```{r}
kableExtra::kable(dat, col.names = c("Y", "Treatment", "$Y_{i}(1)$", "ITE", "$Y_{i}(1)$", "$Y>100$"), escape=FALSE)
```
