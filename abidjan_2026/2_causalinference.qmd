...
set.seed(123)
Z_assign <- complete_ra(12)
Z_assign_2 <- complete_ra(12)

est_diff_label <- function(Z) {
  sprintf(
    "%.2f - %.2f = %.2f",
    mean(Y1[Z == 1]),
    mean(Y0[Z == 0]),
...
po_panel <- function(
  ...
  lang <- deck_lang()
  title_main <- if (identical(lang, "fr")) title_fr else title_en
  title_sub <- if (identical(lang, "both")) title_fr else NULL
  xlab <- if (identical(lang, "fr")) x_fr else x_en
  ylab <- if (identical(lang, "fr")) y_fr else y_en
  title_col <- if (identical(lang, "fr")) COL_FR else COL_EN

  p <- p +
    coord_cartesian(xlim = c(-1.5, N + 0.5), ylim = c(-3, 4.5), clip = "off") +
    labs(title = title_main, subtitle = title_sub, x = xlab, y = ylab) +
    theme_bw() +
    theme(
      plot.title = element_text(
        color = title_col,
        size = 11,
        face = if (identical(lang, "fr")) "italic" else "bold",
        margin = margin(b = 2)
      ),
      ...
      axis.title.x = element_text(
        color = if (identical(lang, "fr")) COL_FR else COL_EN,
        size = 10,
        margin = margin(t = 8)
      ),
...
![](Images/A_small_cup_of_coffee.jpeg){width="80px" fig-align="center"}
...
## We have 12 units | *Nous avons 12 unités* {.smaller}
...
## Each unit has both $Y_i(1)$ and $Y_i(0)$ | *Chaque unité a à la fois* $Y_i(1)$ et $Y_i(0)$  {.smaller}
...
## The treatment effect for $i$ is defined as $\tau_i \equiv Y_i(1) - Y_i(0)$ | *L'effet de traitement pour individu $i$ est défini comme* $\tau_i \equiv Y_i(1) - Y_i(0)$  {.smaller}
...
## Average Treatment Effect $\overline{\tau_i}= \overline{Y_i(1)-Y_i(0)}$ / *L'effet moyen du traitement* (ATE)  $\overline{\tau_i}= \overline{Y_i(1)-Y_i(0)}$  {.smaller}
...
## A random assignment | *Une assignation aléatoire*  {.smaller}
...
## We learn the outcomes and produce an estimate | *Nous apprenons les résultats et produisons une estimation*  {.smaller}
...
#  subtitle_fr = "Différence des moyennes d'échantillon"
  title_en = est_diff_label(Z_assign),
  title_fr = est_diff_label(Z_assign),
)
```
...
#| out-width: 100%
plot_assignment(
  labels = if_else(Z_assign_2 == 1, as.character(Y1), as.character(Y0)),
  Z = Z_assign_2,
#  subtitle_en = "Sample mean difference",
#  subtitle_fr = "Différence des moyennes d'échantillon"
  title_en = est_diff_label(Z_assign_2),
  title_fr = est_diff_label(Z_assign_2)
)
```
...
## Randomization works in expectation | *La randomisation fonctionne en moyenne* {.smaller}
