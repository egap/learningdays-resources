...
## Randomization inference | *Inférence par randomisation*
...
Illustration des $p$-values par inférence de randomisation.
...
Test: a six **never** comes up on this die.
...
Test : un six **ne sort jamais** sur ce dé.
...
Test: the die is **fair**.

- Roll **1000** times.
- Reject if fewer than $x$ sixes or more than $y$ sixes.

What should $x$ and $y$ be? What is the power?
...
Test : le dé est **équitable**.

- Lancer **1000** fois.
- Rejeter si moins de $x$ six ou plus de $y$ six.

Que valent $x$ et $y$ ? Quelle est la puissance ?
...
First, set a rejection rule based on events unlikely under the hypothesis.
...
D'abord, fixer une règle de rejet à partir d'événements improbables sous l'hypothèse.
...
## Equivalent formula | *Formule équivalente*
...
## Equivalence check | *Vérification d'équivalence*

```{r, include = FALSE}
powers <- sapply(seq(0, 1, .05), function(j) {
  c(
    pwrss::pwrss.t.2means(mu1 = j, n2 = 50)$power,
    power_2(j, N = 100)
  )
}) |>
  t() |>
...
# Complex trials | *Essais complexes*
...
## Declare a design | *Déclarer une conception*
...
## Power over N and b | *Puissance selon N et b*
...
## Multiple diagnosands | *Plusieurs diagnosandes*
