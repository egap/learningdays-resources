...
df <- fabricate(N = 100, 
               Coffee = rep(0:1, N/2), 
               Sporty_Sportive = rnorm(N), 
               Energy = .2 + .4*Coffee  + .3* Sporty_Sportive + rnorm(N))
```
...
$$Y_i = \hat{\alpha_0} + \hat{\beta_1} Z_i + e_i$$
...
- $\hat{\alpha_0}$ is the average outcome in the control group.
...
- $\hat{\alpha_0}$ est le résultat moyen des unités assignées au contrôle.
...
lm_robust(Energy ~ Coffee  + Sporty_Sportive, data = df) |> texreg::htmlreg(inclide.ci = FALSE) 
